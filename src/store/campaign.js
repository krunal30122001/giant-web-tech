...
import { fetchCampaignData, uploadStory, fetchPromoCodeData } from '@/api/campaignService'; // Adjust the path as needed
...
export const useCampaignStore = defineStore('campaign', {
    ...
    getters: {
        ...
        shoppingCartPages: (state) => state.campaignData?.theme?.shopping_cart_pages,
        inscriptionExamples: (state) => state.campaignData?.theme?.inscription_examples || [],
        contactInfo: (state) => state.campaignData?.theme?.contact || {},
        storiesData: (state) => state.campaignData?.theme?.stories?.stories_data || [],
        faqData: (state) => state.campaignData?.theme?.faq || [],
        termsConditions: (state) => state.campaignData?.theme?.terms_conditions?.terms_conditions || null,
        confirmationPage: (state) => state.campaignData?.theme?.confirmation_page || null,
        additionField: (state) => state.campaignData?.theme?.additionField || null,

        locationpreferenceAllowed: (state) => {
            return state.campaignData?.campaign?.options?.is_locationpreference_allowed;
        },
        multilogoAllowed: (state) => {
            return state.campaignData?.campaign?.options?.is_multilogo_allowed;
        },
        inscriptionText: (state) => {
            return state.campaignData?.campaign?.options?.inscription?.is_all_uppercase || false;
        },

        // Convenience amount calculation based on `is_allowed_oos`
        convenienceAmount: (state) => {
            const convenienceOptions = state.campaignData?.campaign?.options?.convenience;
